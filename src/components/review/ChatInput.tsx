...
export function ChatInput({ onSend, thinkingMode, onToggleThinking, disabled }: Props) {
  ...
  <form
    onSubmit={submit}
    className="w-full min-w-0 rounded-[1.75rem] border border-border bg-card/95 p-4 pl-5 shadow-cozy backdrop-blur"
  >
    <div className="flex min-w-0 items-end gap-4">
      <textarea
        ...
        className="max-h-40 min-w-0 flex-1 resize-none bg-transparent py-4 text-sm outline-none placeholder:text-muted-foreground"
      />
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <button
          ...
          aria-label="Send"
          disabled={disabled}
          className="grid h-9 w-9 place-items-center rounded-full bg-primary text-primary-foreground shadow-cozy transition hover:shadow-float disabled:opacity-50"
        >
